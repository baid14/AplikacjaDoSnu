...
if($a==2){
	$wszystko_OK=false;
	$_SESSION['e_a']="Nie wybrano parametru A  !";
	
}

if($b==2){
	$wszystko_OK=false;
	$_SESSION['e_b']="Nie wybrano parametru B  !";
}

if($c==2){
	$wszystko_OK=false;
	$_SESSION['e_c']="Nie wybrano parametru C !";
	
}

if($d==2){
	$wszystko_OK=false;
	$_SESSION['e_d']="Nie wybrano parametru D  !";
}

if($e==2){
	$wszystko_OK=false;
	$_SESSION['e_e']="Nie wybrano parametru E  !";
	
}

if($f==2){
	$wszystko_OK=false;
	$_SESSION['e_f']="Nie wybrano parametru F  !";
}
if($g==2){
	$wszystko_OK=false;
	$_SESSION['e_g']="Nie wybrano parametru G  !";
	
}

if($h==2){
	$wszystko_OK=false;
	$_SESSION['e_h']="Nie wybrano parametru H  !";
}
...
$dateNow = date('Y-m-d');
	...
	else
	{
		...
		sprintf("SELECT id_Analiza_dnia FROM uzytkownicy,Analiza_dnia WHERE Analiza_dnia.id_uzytkownicy=uzytkownicy.id AND uzytkownicy.id='%s' AND data='$data'",
		mysqli_real_escape_string($polaczenie,$idd22))))
		{
			...
			if ($polaczenie->query(sprintf("UPDATE Analiza_dnia  SET  par1='$a', 	par2='$b', 	par3='$c', 	par4='$d', 	par5='$e', 	par6='$f', 	par7='$g', 	par8='$h', 	notatka='%s' WHERE id_uzytkownicy='$idd22' AND data='$data'",
		mysqli_real_escape_string($polaczenie,$notatka))))
					...
					else
					{
						throw new Exception($polaczenie->error);
									...
									if ($polaczenie->query(sprintf("INSERT INTO Analiza_dnia(id_Analiza_dnia, 	id_uzytkownicy, 	par1, 	par2, 	par3, 	par4, 	par5, 	par6, 	par7, 	par8, 	notatka, 	data) VALUES (NULL,'$idd22','$a','$b','$c','$d','$e','$f','$g','$h','%s','$data')",
		mysqli_real_escape_string($polaczenie,$notatka))))
					{
						$_SESSION['udanawprowadzenie']=true;
...
<textarea rows="10" name="notatka"></textarea>	
